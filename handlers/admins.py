...
async def pause(_, message: Message):
    if (
        ...
        await message.reply_text("🛰️=▶️ Paused!")
...
async def stop(_, message: Message):
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("🛰️=❗ Nothing is streaming!")
    else:
        try:
...
async def skip(_, message: Message):
    global que
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("🛰️=❗ Nothing is playing to skip!")
    else:
...
async def admincache(client, message: Message):
    ...
    #await message.reply_text("🛰️=❇️ Admin cache refreshed!")
